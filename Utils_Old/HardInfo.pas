...
function GetCPUVendor : string; //获取CPU生产厂家函数
...
function GetScsisn: string; //获取IDE硬盘序列号函数 需要管理员权限
...
function _GetCPUID: TCPUID; assembler; register;
asm
...
begin
  Result  :=  _GetCPUID;
  //处理多核
  Result[2] :=  Result[2] and $00FFFFFF;
end;
...
//用getscsisn既可得到硬盘序列号
function GetIdeDiskSerialNumber: PAnsiChar;
//获取第一个IDE硬盘的序列号
...
begin
  ...
  if DeviceIoControl(DeviceHandle, $0004D004, @sptwb, SizeOf
    ...
    and ((PAnsiChar(@sptwb.bDataBuf) + 1)^ = #$80)
    then
    SetString(Result, PAnsiChar(@sptwb.bDataBuf) + 4,
      Ord((PAnsiChar(@sptwb.bDataBuf) + 3)^));
end;
