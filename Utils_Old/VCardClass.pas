...
uses
  Classes, SysUtils, IdBaseComponent, IdGlobal;
...
type
  ...
  TZsVCardEmbeddedObject = class(TPersistent)
  protected
    FObjectType: String;
    FObjectURL: String;
  ...
  TZsVCardBusinessInfo = class(TPersistent)
  protected
    FTitle: String;
    FRole: String;
  ...
  TZsVCardGeog = class(TPersistent)
  protected
    FLatitude: Real;
    FLongitude: Real;
    FTimeZoneStr: String;
  ...
  TIdPhoneAttributes = set of (tpaHome, tpaVoiceMessaging, tpaWork, tpaPreferred, tpaVoice, tpaFax,
  ...
  { Since a person can have more than one address, we put them into this
    collection }
  TZsVCardTelephones = class(TOwnedCollection)
  protected
  ...
  TIdCardAddressAttributes = set of (tatHome, tatDomestic, tatInternational, tatPostal, tatParcel,
    tatWork, tatPreferred);
  ...
  published
    ...
    property AddressAttributes: TIdCardAddressAttributes read FAddressAttributes write
      FAddressAttributes;
  ...
  TZsVCardAddresses = class(TOwnedCollection)
  protected
  ...
  TZsVCardMailingLabelItem = class(TCollectionItem)
  private
    FAddressAttributes: TIdCardAddressAttributes;
  ...
  { This type holds the }
  TZsVCardMailingLabels = class(TOwnedCollection)
  protected
    function GetItem(Index: Integer): TZsVCardMailingLabelItem;
    procedure SetItem(Index: Integer; const Value: TZsVCardMailingLabelItem);
  public
    constructor Create(AOwner: TPersistent); reintroduce;
    function Add: TZsVCardMailingLabelItem;
    property Items[Index: Integer]: TZsVCardMailingLabelItem read GetItem write SetItem; default;
  end;

  { This type is used to indicate the type E-Mail indicated in the VCard
    which can be of several types }
  TZsVCardEMailType = (ematAOL, { America On-Line }
  ...
  TZsVCardEMailItem = class(TCollectionItem)
  protected
    FEMailType: TZsVCardEMailType;
    FPreferred: Boolean;
    FAddress: String;
  public
    constructor Create(Collection: TCollection); override;
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property EMailType: TZsVCardEMailType read FEMailType write FEMailType;
  ...
  TZsVCardEMailAddresses = class(TOwnedCollection)
  protected
    function GetItem(Index: Integer): TZsVCardEMailItem;
    procedure SetItem(Index: Integer; const Value: TZsVCardEMailItem);
  public
    constructor Create(AOwner: TPersistent); reintroduce;
    function Add: TZsVCardEMailItem;
    property Items[Index: Integer]: TZsVCardEMailItem read GetItem write SetItem; default;
  end;

  TZsVCardName = class(TPersistent)
  protected
    FFirstName: String;
    FSurName: String;
  ...
  TZsVCard = class(TIdBaseComponent)
  private
  protected
    FComments: TStrings;
    FCategories: TStrings;
    FBusinessInfo: TZsVCardBusinessInfo;
    FGeography: TZsVCardGeog;
    FFullName: TZsVCardName;
    FRawForm: TStrings;
    FURLs: TStrings;
    FEMailProgram: String;
    FEMailAddresses: TZsVCardEMailAddresses;
    FAddresses: TZsVCardAddresses;
    FMailingLabels: TZsVCardMailingLabels;
    FTelephones: TZsVCardTelephones;
    FVCardVersion: Real;
    FProductID: String;
    FUniqueID: String;
    ...
    FBirthDay: TDateTime;
    FPhoto: TZsVCardEmbeddedObject;
    FLogo: TZsVCardEmbeddedObject;
    FSound: TZsVCardEmbeddedObject;
    FKey: TZsVCardEmbeddedObject;
    FDmCardID: String;
  ...
  public
    ...
    function ReadFromTStrings(s: TStrings): Boolean;
    { This is the raw form of the VCard }
    property RawForm: TStrings read FRawForm;
    property DmCardID: String read FDmCardID;
  published
    ...
    property FullName: TZsVCardName read FFullName write FFullName;
    ...
    property EMailAddresses: TZsVCardEMailAddresses read FEMailAddresses;
    { This is a list of telephone numbers }
    property Telephones: TZsVCardTelephones read FTelephones;
    { This is busines related information on a VCard }
    property BusinessInfo: TZsVCardBusinessInfo read FBusinessInfo;
    ...
    property Addresses: TZsVCardAddresses read FAddresses;
    { This is a list of mailing labels }
    property MailingLabels: TZsVCardMailingLabels read FMailingLabels;
    ...
    property Photo: TZsVCardEmbeddedObject read FPhoto;
    { Organization's logo }    { Do not Localize }
    property Logo: TZsVCardEmbeddedObject read FLogo;
    { A sound associated with the VCard such as how to pronounce a person's name
      or something cute }
    property Sound: TZsVCardEmbeddedObject read FSound;
    { This is for an encryption key such as S/MIME, VeriSign, or PGP }
    property Key: TZsVCardEmbeddedObject read FKey;
  end;
...
const
  VCardProperties: array [1 .. 29] of string = ('FN', 'N', 'NICKNAME', 'PHOTO', { Do not Localize }
    ...
    'UID', 'VERSION', 'CLASS', 'KEY', 'DMCARDID');
  { Do not Localize }
  { These constants are for testing the VCard for E-Mail types.
    Don't alter these }    { Do not Localize }
const
  EMailTypePropertyParameter: array [1 .. 12] of string = ('AOL', { America On-Line }
    { Do not Localize }
...
Procedure ParseDelinatorToTStrings(strs: TStrings; str: String; deliniator: char = ',');
{ Do not Localize }
begin
...
procedure ParseOrg(OrgObj: TZsVCardBusinessInfo; OrgStr: String);
begin
  { Organization name }
...
{ This parses the geography latitude and longitude from GeogStr and
  puts it in Geog }
procedure ParseGeography(Geog: TZsVCardGeog; GeogStr: String);
begin
  { Latitude }
...
const
  TelephoneTypePropertyParameter: array [0 .. 13] of string = ('HOME', 'MSG', 'WORK', 'PREF',
    'VOICE', 'FAX', { Do not Localize }
  ...
  try
    ...
    begin
      case IdGlobal.PosInStrArray(Attribs[idx], TelephoneTypePropertyParameter) of
        { home }
        0:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaHome];
        { voice messaging }
        1:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaVoiceMessaging];
        { work }
        2:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaWork];
        { preferred }
        3:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaPreferred];
        { Voice }
        4:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaVoice];
        { Fax }
        5:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaFax];
        { Cellular phone }
        6:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaCellular];
        { Video conferancing number }
        7:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaVideo];
        { Bulleton Board System (BBS) telephone number }
        8:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaBBS];
        { MODEM Connection number }
        9:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaModem];
        { Car phone number }
        10:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaCar];
        { ISDN Service Number }
        11:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaISDN];
        { personal communication services telephone number }
        12:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaPCS];
        { pager }
        13:
          PhoneObj.PhoneAttributes := PhoneObj.PhoneAttributes + [tpaPager];
      end;
      inc(idx);
...
var
  ...
  idx: Integer;
const
  AttribsArray: Array [0 .. 6] of String = ('HOME', 'DOM', 'INTL', 'POSTAL', 'PARCEL', 'WORK',
    'PREF'); { Do not Localize }
begin
  ...
  try
    ...
    begin
      case IdGlobal.PosInStrArray(Attribs[idx], AttribsArray) of
        { home }
        0:
          AddressObj.AddressAttributes := AddressObj.AddressAttributes + [tatHome];
        { domestic }
        1:
          AddressObj.AddressAttributes := AddressObj.AddressAttributes + [tatDomestic];
        { international }
        2:
          AddressObj.AddressAttributes := AddressObj.AddressAttributes + [tatInternational];
        { Postal }
        3:
          AddressObj.AddressAttributes := AddressObj.AddressAttributes + [tatPostal];
        { Parcel }
        4:
          AddressObj.AddressAttributes := AddressObj.AddressAttributes + [tatParcel];
        { Work }
        5:
          AddressObj.AddressAttributes := AddressObj.AddressAttributes + [tatWork];
        { Preferred }
        6:
          AddressObj.AddressAttributes := AddressObj.AddressAttributes + [tatPreferred];
      end;
      inc(idx);
    end; // while idx < Attribs.Count do
    if (Attribs.Count = 0) then
...
{ This parses LabelStr and places the attributes in TZsVCardMailingLabelItem }
Procedure ParseMailingLabel(LabelObj: TZsVCardMailingLabelItem; LabelStr: String);
var
  Value: String;
  Attribs: TStringList;
  idx: Integer;
  ...
  try
    ...
    begin
      case IdGlobal.PosInStrArray(Attribs[idx], AttribsArray) of
        { home }
        0:
          LabelObj.AddressAttributes := LabelObj.AddressAttributes + [tatHome];
        { domestic }
        1:
          LabelObj.AddressAttributes := LabelObj.AddressAttributes + [tatDomestic];
        { international }
        2:
          LabelObj.AddressAttributes := LabelObj.AddressAttributes + [tatInternational];
        { Postal }
        3:
          LabelObj.AddressAttributes := LabelObj.AddressAttributes + [tatPostal];
        { Parcel }
        4:
          LabelObj.AddressAttributes := LabelObj.AddressAttributes + [tatParcel];
        { Work }
        5:
          LabelObj.AddressAttributes := LabelObj.AddressAttributes + [tatWork];
        { Preferred }
        6:
          LabelObj.AddressAttributes := LabelObj.AddressAttributes + [tatPreferred];
      end;
      inc(idx);
    end; // while idx < Attribs.Count do
    { Default Values }
...
{ This parses the Name and places the name in the TZsVCardName }
Procedure ParseName(NameObj: TZsVCardName; NameStr: String);
var
  OtherNames: String;
...
Procedure ParseEMailAddress(EMailObj: TZsVCardEMailItem; EMailStr: String);
var
  Value: String;
  Attribs: TStringList;
  idx: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      case ps of
        0:
          EMailObj.EMailType := ematAOL; { America On-Line }
        1:
          EMailObj.EMailType := ematAppleLink; { AppleLink }
        2:
          EMailObj.EMailType := ematATT; { AT&T Mail }
        3:
          EMailObj.EMailType := ematCIS; { CompuServe Information Service }
        4:
          EMailObj.EMailType := emateWorld; { eWorld }
        5:
          EMailObj.EMailType := ematInternet; { Internet SMTP (default) }
        6:
          EMailObj.EMailType := ematIBMMail; { IBM Mail }
        7:
          EMailObj.EMailType := ematMCIMail; { Indicates MCI Mail }
        8:
          EMailObj.EMailType := ematPowerShare; { PowerShare }
        9:
          EMailObj.EMailType := ematProdigy; { Prodigy information service }
        10:
          EMailObj.EMailType := ematTelex; { Telex number }
        11:
          EMailObj.EMailType := ematX400; { X.400 service }
      end; // case ps of
      inc(idx);
    end; // while (idx < Attribs.Count ) do
  finally
...
constructor TZsVCard.Create(AOwner: TComponent);
begin
  inherited;
  FPhoto := TZsVCardEmbeddedObject.Create;
  FLogo := TZsVCardEmbeddedObject.Create;
  FSound := TZsVCardEmbeddedObject.Create;
  FKey := TZsVCardEmbeddedObject.Create;
  FComments := TStringList.Create;
  FCategories := TStringList.Create;
  FBusinessInfo := TZsVCardBusinessInfo.Create;
  FGeography := TZsVCardGeog.Create;
  FFullName := TZsVCardName.Create;
  FRawForm := TStringList.Create;
  FEMailAddresses := TZsVCardEMailAddresses.Create(Self);
  FAddresses := TZsVCardAddresses.Create(Self);
  FTelephones := TZsVCardTelephones.Create(Self);
  FURLs := TStringList.Create;
  FMailingLabels := TZsVCardMailingLabels.Create(Self);
end;

destructor TZsVCard.Destroy;
begin
  FreeAndNil(FKey);
  FreeAndNil(FPhoto);
  FreeAndNil(FLogo);
...
function TZsVCard.ReadFromTStrings(s: TStrings): Boolean;
var
...
begin
  ...
  idx := 0;
  embedded := 0;
  while (idx < s.Count) and (Trim(UpperCase(s[idx])) <> 'BEGIN:VCARD') do { Do not Localize }
  begin
    inc(idx);
  end; // while ..

  result := idx < s.Count - 1;
  ...
  begin
    ...
    begin
      ...
      else if UpperCase(Trim(s[idx])) <> 'BEGIN:VCARD' then { Do not Localize }
      begin
...
procedure TZsVCard.SetCategories(Value: TStrings);
begin
  FCategories.Assign(Value);
end;

procedure TZsVCard.SetComments(Value: TStrings);
begin
  FComments.Assign(Value);
end;

procedure TZsVCard.SetURLs(Value: TStrings);
begin
  FURLs.Assign(Value);
end;

procedure TZsVCard.SetVariablesAfterRead;
var
  idx: Integer;
  OrigLine: String;
  Line: String;
  ...
  begin
    ...
    inc(idx);
    while (idx < FRawForm.Count) and ((Length(FRawForm[idx]) > 0) and (FRawForm[idx][1] = ' ') or
        (FRawForm[idx][1] = #9)) do { Do not Localize }
    begin
  ...
  Procedure ParseEmbeddedObject(EmObj: TZsVCardEmbeddedObject; StLn: String);
  var
    Value: String;
    Attribs: TStringList;
    idx2: Integer;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        inc(idx);
        while (idx < FRawForm.Count) and ((Length(FRawForm[idx]) > 0) and (FRawForm[idx][1] = ' ')
            or (FRawForm[idx][1] = #9)) do { Do not Localize }
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      data := QuotedPrintableDecode_Str(data);
    ...
    case PosInStrArray(Test, VCardProperties) of
      { 'FN' }    { Do not Localize }
      0:
        FFullName.FormattedName := Line + UnfoldLines;
      { 'N' }    { Do not Localize }
      1:
        ParseName(FFullName, Line + UnfoldLines);
      { 'NICKNAME' }    { Do not Localize }
      2:
        ParseDelinatorToTStrings(FFullName.NickNames, Line + UnfoldLines);
      { 'PHOTO' }    { Do not Localize }
      3:
        ParseEmbeddedObject(FPhoto, Line);
      { 'BDAY' }    { Do not Localize }
      4:
        FBirthDay := ParseDateTimeStamp(Line + UnfoldLines);
      { 'ADR' }    { Do not Localize }
      5:
        ParseAddress(FAddresses.Add, Line + UnfoldLines);
      { 'LABEL' }    { Do not Localize }
      6:
        ParseMailingLabel(FMailingLabels.Add, Line + UnfoldLines);
      { 'TEL' }    { Do not Localize }
      7:
        ParseTelephone(FTelephones.Add, Line + UnfoldLines);
      { 'EMAIL' }    { Do not Localize }
      8:
        ParseEMailAddress(FEMailAddresses.Add, Line + UnfoldLines);
      { 'MAILER' }    { Do not Localize }
      9:
        FEMailProgram := Line + UnfoldLines;
      { 'TZ' }    { Do not Localize }
      10:
        FGeography.TimeZoneStr := Line + UnfoldLines;
      { 'GEO' }    { Do not Localize }
      11:
        ParseGeography(FGeography, Line + UnfoldLines);
      { 'TITLE' }    { Do not Localize }
      12:
        FBusinessInfo.Title := Line + UnfoldLines;
      { 'ROLE' }    { Do not Localize }
      13:
        FBusinessInfo.Role := Line + UnfoldLines;
      { 'LOGO' }    { Do not Localize }
      14:
        ParseEmbeddedObject(FLogo, Line);
      { 'AGENT' }    { Do not Localize }
      15:
        ProcessAgent;
      { 'ORG' }    { Do not Localize }
      16:
        ParseOrg(FBusinessInfo, Line + UnfoldLines);
      { 'CATEGORIES' }    { Do not Localize }
      17:
        ParseDelinatorToTStrings(FCategories, Line + UnfoldLines);
      { 'NOTE' }    { Do not Localize }
      18:
        FComments.Add(Line + UnfoldLines);
      { 'PRODID' }    { Do not Localize }
      19:
        FProductID := Line + UnfoldLines;
      { 'REV' }    { Do not Localize }
      20:
        FLastRevised := ParseDateTimeStamp(Line + UnfoldLines);
      { 'SORT-STRING' }    { Do not Localize }
      21:
        FFullName.SortName := Line + UnfoldLines;
      { 'SOUND' }    { Do not Localize }
      22:
        ParseEmbeddedObject(FSound, Line);
      { 'URL' }    { Do not Localize }
      23:
        AddValueToStrings(FURLs, Line + UnfoldLines);
      { 'UID' }    { Do not Localize }
      24:
        FUniqueID := Line + UnfoldLines;
      { 'VERSION' }    { Do not Localize }
      25:
        FVCardVersion := StrToFloat(Line + UnfoldLines);
      { 'CLASS' }    { Do not Localize }
      26:
        FClassification := Line + UnfoldLines;
      { 'KEY' }    { Do not Localize }
      27:
        ParseEmbeddedObject(FKey, Line);
      { 'DMCARDID' }
      28:
        FDmCardID := Line + UnfoldLines;
    end;
    inc(idx);
  end; // while idx < FRawForm.Count do
end;

{ TZsVCardEMailAddresses }

function TZsVCardEMailAddresses.Add: TZsVCardEMailItem;
begin
  result := TZsVCardEMailItem( inherited Add);
end;

constructor TZsVCardEMailAddresses.Create(AOwner: TPersistent);
begin
  inherited Create(AOwner, TZsVCardEMailItem);
end;

function TZsVCardEMailAddresses.GetItem(Index: Integer): TZsVCardEMailItem;
begin
  result := TZsVCardEMailItem( inherited Items[Index]);
end;

procedure TZsVCardEMailAddresses.SetItem(Index: Integer; const Value: TZsVCardEMailItem);
begin
...
procedure TZsVCardEMailItem.Assign(Source: TPersistent);
var
  EMail: TZsVCardEMailItem;
begin
  ...
  begin
    EMail := TZsVCardEMailItem(Source);
...
constructor TZsVCardEMailItem.Create(Collection: TCollection);
begin
  inherited;
...
function TZsVCardAddresses.Add: TIdCardAddressItem;
begin
  result := TIdCardAddressItem( inherited Add);
end;

constructor TZsVCardAddresses.Create(AOwner: TPersistent);
begin
  inherited Create(AOwner, TIdCardAddressItem);
end;

function TZsVCardAddresses.GetItem(Index: Integer): TIdCardAddressItem;
begin
  result := TIdCardAddressItem( inherited Items[Index]);
end;

procedure TZsVCardAddresses.SetItem(Index: Integer; const Value: TIdCardAddressItem);
begin
  inherited SetItem(Index, Value);
end;
...
function TZsVCardTelephones.Add: TIdCardPhoneNumber;
begin
  result := TIdCardPhoneNumber( inherited Add);
end;

constructor TZsVCardTelephones.Create(AOwner: TPersistent);
begin
  inherited Create(AOwner, TIdCardPhoneNumber);
end;

function TZsVCardTelephones.GetItem(Index: Integer): TIdCardPhoneNumber;
begin
  result := TIdCardPhoneNumber( inherited Items[Index]);
end;

procedure TZsVCardTelephones.SetItem(Index: Integer; const Value: TIdCardPhoneNumber);
begin
  inherited SetItem(Index, Value);
end;
...
constructor TZsVCardName.Create;
begin
  inherited;
...
destructor TZsVCardName.Destroy;
begin
...
procedure TZsVCardName.SetNickNames(Value: TStrings);
begin
  FNickNames.Assign(Value);
end;

procedure TZsVCardName.SetOtherNames(Value: TStrings);
begin
  FOtherNames.Assign(Value);
end;

{ TZsVCardBusinessInfo }

constructor TZsVCardBusinessInfo.Create;
begin
  inherited;
  FDivisions := TStringList.Create;
end;

destructor TZsVCardBusinessInfo.Destroy;
begin
  FreeAndNil(FDivisions);
  inherited;
end;

procedure TZsVCardBusinessInfo.SetDivisions(Value: TStrings);
begin
  FDivisions.Assign(Value);
end;

{ TZsVCardMailingLabelItem }

procedure TZsVCardMailingLabelItem.Assign(Source: TPersistent);
var
  lbl: TZsVCardMailingLabelItem;
begin
  if ClassType <> Source.ClassType then
  begin
    inherited
  ...
  begin
    lbl := TZsVCardMailingLabelItem(Source);
    AddressAttributes := lbl.AddressAttributes;
    MailingLabel.Assign(lbl.MailingLabel);
  end;
end;

constructor TZsVCardMailingLabelItem.Create(Collection: TCollection);
begin
  inherited;
  FMailingLabel := TStringList.Create;
end;

destructor TZsVCardMailingLabelItem.Destroy;
begin
  FreeAndNil(FMailingLabel);
  inherited;
end;

procedure TZsVCardMailingLabelItem.SetMailingLabel(Value: TStrings);
begin
  FMailingLabel.Assign(Value);
end;

{ TZsVCardMailingLabels }

function TZsVCardMailingLabels.Add: TZsVCardMailingLabelItem;
begin
  result := TZsVCardMailingLabelItem( inherited Add);
end;

constructor TZsVCardMailingLabels.Create(AOwner: TPersistent);
begin
  inherited Create(AOwner, TZsVCardMailingLabelItem);
end;

function TZsVCardMailingLabels.GetItem(Index: Integer): TZsVCardMailingLabelItem;
begin
  result := TZsVCardMailingLabelItem( inherited GetItem(Index));
end;

procedure TZsVCardMailingLabels.SetItem(Index: Integer; const Value: TZsVCardMailingLabelItem);
begin
  inherited SetItem(Index, Value);
end;
...
constructor TZsVCardEmbeddedObject.Create;
begin
  inherited;
  FEmbeddedData := TStringList.Create;
end;

destructor TZsVCardEmbeddedObject.Destroy;
begin
  FreeAndNil(FEmbeddedData);
  inherited;
end;

procedure TZsVCardEmbeddedObject.SetEmbeddedData(const Value: TStrings);
begin
