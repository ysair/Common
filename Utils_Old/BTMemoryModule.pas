...
var
  ...
  hfunc : function (const AInStr : PChar; const AOutStr : PChar; const AOutSize : Integer):integer; stdcall;
  btMM: PBTMemoryModule;
begin
  stream := TResourceStream.Create(HInstance, RES_360SignDll, RESTYPE_DLL);
  try
    btMM := BTMemoryLoadLibary(stream.Memory, stream.Size);
    @hFunc  :=  BTMemoryGetProcAddress(btMM, 'Test');
    hFunc('abc', @ps[0], 20);
    Caption :=  StrPas(ps);
